...
public sealed class BlipCommandService(ILogger logger) : IBlipCommandService
{
  ...
  public async Task<IList<Item>> GetCustomRepliesAsync(string shortName, string botAuthKey)
  {
    try
    {
      var command = new Command
      {
        Uri = $"/replies?$skip=0&$take=8",
        Method = CommandMethod.Get,
      ...
      var json = JsonConvert.SerializeObject(result.Resource);

      var customReplies = JsonConvert.DeserializeObject<IList<Item>>(json);
  ...
  public async Task PublishCustomRepliesAsync(string shortName, string botAuthKey, IList<Item> items, string categoryId = null)
  {
    var customReplyId = categoryId;

    if (string.IsNullOrWhiteSpace(categoryId))
      customReplyId = Guid.NewGuid().ToString();
    ...
    if (result.Status == CommandStatus.Success)
    {
      logger.Information("Success to update custom reply for {DealerName}", shortName);
    }
    else
    {
      logger.Error("Error to update custom reply for {DealerName}: {Reason}", shortName, result.Reason?.Description);
    }
  }
